...
| 9 | Workspace scoping + actor access checks | Very High | Medium | Implemented (v1) — workspace membership enforced and Auth.js middleware injects signed actor identity into legacy services |
...
| 18 | RBAC permission helpers | High | Medium | Implemented foundation (v1) — role-permission matrix + enforcement utilities exist; broader sensitive-route wiring remains |
| 19 | Calendar/HubSpot OAuth stubs | Medium | High | Stubbed (v1) — interfaces defined, awaiting provider credentials |
| 20 | Session-backed auth + protected routes | Very High | Medium | Implemented foundation (v1) — Auth.js JWT sessions, branded sign-in, Google/LinkedIn/dev providers, and middleware protection |
