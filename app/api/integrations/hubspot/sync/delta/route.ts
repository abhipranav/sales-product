...
import {
  parseRunDeltaSyncInput,
  runHubspotDeltaScheduler
...
export async function POST(request: Request) {
  try {
    ...
    const payload = parseRunDeltaSyncInput(body);
    const result = await runHubspotDeltaScheduler(payload, actor);
    return NextResponse.json(result, { status: result.status === "synced" ? 201 : 200 });
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: "Invalid delta sync payload.", details: error.flatten() }, { status: 400 });
    }
    ...
    console.error("HubSpot delta sync failed", error);
    return NextResponse.json({ error: "Failed to run HubSpot delta sync." }, { status: 500 });
  }
}
